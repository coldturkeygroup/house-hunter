...
class HouseHunter_Admin {
	...
	/**
	 * Basic constructor for the House Hunter Admin class
	 *
	 * @param string $file
	 */
	...
	public function add_menu_item()
	{
		...
		add_submenu_page( 'edit.php?post_type=' . $this->token, 'House Hunter Settings', 'Settings', 'manage_options', $this->token . '_settings', [
			$this,
			'settings_page'
		] );
	}
	...
	public function admin_notices()
	{
		...
		if ( $wp_version < 3.5 ) {
			?>
			<div class="error">
				<p><?php printf( __( '%1$sHouse Hunter%2$s requires WordPress 3.5 or above in order to function correctly. You are running v%3$s - please update now.', $this->token ), '<strong>', '</strong>', $wp_version ); ?></p>
			</div>
		<?php
		}
	}
	...
	public function register_settings()
	{
		...
		// Add settings fields
		add_settings_field( $this->token . '_slug', __( 'URL slug for house hunter pages:', $this->token ), [
			$this,
			'slug_field'
	...
	{
		echo '<p>' . __( 'These are a few simple settings for setting up your house hunter pages.', $this->token ) . '</p>';
	}
	...
	public function slug_field()
	{
		...
		echo '<input id="slug" type="text" name="' . $this->token . '_slug" value="' . $data . '"/>
				<label for="slug"><span class="description">' . sprintf( __( 'Provide a custom URL slug for the house hunter pages.', $this->token ) ) . '</span></label>';
	}
